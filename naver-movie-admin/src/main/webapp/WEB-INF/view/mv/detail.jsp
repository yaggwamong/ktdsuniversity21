...
<script type="text/javascript">
	...
	function addPplFn(message){
		...
		var added = $("<input type='hidden' name='mvPplList["+ len+ "].added' />");
		added.val("true")
		itemDiv.append(added);
	...
	$().ready(function(){
		...
		$(".del-gnr-item-btn").click(function(){	
			...
			$(this).remove(); //삭제를 또 할 수 있기때문에 버튼 지우기
		})	
		...
		$(".del-ppl-item-btn").click(function(){
			...
			var index = $(this).data("index");
			var deleted =$("<input type ='hidden' name='mvPplList["+index+"].deleted' />");
		...
		$(".rspnsbltRolNm").keyup(function(){
			...
			if(orgnName==nowName){
				...
				// TODO input type = hidden name="mvPplList[index].modified remove" 
			}
			else {
				var modifiedDom = parent.find("input.mdfy");
				if(modifiedDom.length ==0){
				var modified = $("<input type ='hidden' class='mdfy' name='mvPplList["+ index+ "].modified' />");
					...
					<form id="create_form" enctype="multipart/form-data">
						...
						<div class="create-group">
							...
							<select id="grd" name="grd">
								...
								<option value="19" ${mvVO.grd eq '19' ? 'selected' : ''}>청소년관람불가</option>		
							</select>
						</div>
						...
						<div class="create-group">
							...
							<div>
								...
								<div class="items">
									<c:forEach items="${mvVO.mvPplList}" var="ppl" varStatus="index">
										<c:if test="${ppl.mssn eq 'DRCTR'}">
											<div class='mvppl-item ${ppl.mvPplId}'>
												<input type='hidden' name='mvPplList[${index.index}].mvPplId' value="${ppl.mvPplId}"/>
												<input type='hidden' name='mvPplList[${index.index}].mssn' placeholder ='임무' value="${ppl.mssn}"/>
												<input type='text'  
													   data-index ="${index.index}" 
												       data-orgn-name ="${ppl.rspnsbltRolNm}" 
												       name='mvPplList[${index.index}].rspnsbltRolNm' 
												       placeholder ='역할명' 
												       value="${ppl.rspnsbltRolNm}"
												       class="rspnsbltRolNm"/>
												<span>${ppl.mvPplVO.nm}</span>
												<button class="del-ppl-item-btn" 
														data-prdcprtcptnid="${ppl.prdcPrtcptnId}"
														data-index= "${index.index}" >X</button>
											</div>
										</c:if>
									</c:forEach>									
								</div>
